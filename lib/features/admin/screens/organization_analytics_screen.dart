...
class OrganizationAnalyticsScreen extends StatefulWidget {
  ...
  @override
  State<OrganizationAnalyticsScreen> createState() => _OrganizationAnalyticsScreenState();
}

class _OrganizationAnalyticsScreenState extends State<OrganizationAnalyticsScreen> {
  ...
  Future<void> _fetchDashboardData() async {
    ...
    try {
      ...
      final goalsResponse = await _supabase.client
          .from('goals')
          .select('id, status, employee_id, quarterly_checkins(id, quarter, status), users(name, email)');
      ...
      for (var g in goals) {
        ...
        final checkins = List<Map<String, dynamic>>.from(g['quarterly_checkins'] ?? []);
        ...
        employeeMap[empId]!['totalGoals'] = (employeeMap[empId]!['totalGoals'] as int) + 1;
        employeeMap[empId]!['checkinsCompleted'] = (employeeMap[empId]!['checkinsCompleted'] as int) + checkins.length;
      }

      _employeeCheckinStatus = employeeMap.values.toList();
      _totalUsers = _employeeCheckinStatus.length;
      _checkinsDone = _employeeCheckinStatus.where((e) => (e['checkinsCompleted'] as int) > 0).length;
    } catch (e) {
          ...
          : SingleChildScrollView(
              ...
              child: ConstrainedBox(
                ...
                child: Column(
                  ...
                  children: [
                    Text('Completion Dashboard', style: AppTypography.headlineLg),
                    const SizedBox(height: 4),
                    Text('Real-time check-in and goal completion status across the organization.',
                    ...
                    Row(
                      children: [
                        _KpiCard(title: 'Total Employees', value: '$_totalUsers', icon: Icons.people_outline),
                        const SizedBox(width: AppSpacing.md),
                        _KpiCard(title: 'Check-ins Filed', value: '$_checkinsDone / $_totalUsers', icon: Icons.check_circle_outline),
                        const SizedBox(width: AppSpacing.md),
                        _KpiCard(title: 'Total Goals', value: '$_totalGoals', icon: Icons.track_changes_outlined),
                      ],
                    ),
                    const SizedBox(height: 32),
                    ...
                    Text('Employee Check-in Status', style: AppTypography.headlineMd),
                    const SizedBox(height: 16),
                    Container(
                      decoration: BoxDecoration(
                        color: AppColors.surfaceWhite,
                            ...
                            return Column(
                              children: [
                                _TableRow(
                                  ...
                                  email: emp['email'],
                                  goals: '$goals',
                                  checkins: checkins > 0 ? '✅ $checkins filed' : '⚠️ None',
                                  isDone: checkins > 0,
                                ),
                                const Divider(height: 1),
                              ],
    ...
    return Padding(
      ...
      child: Row(
        children: [
          ...
          Expanded(flex: 3, child: Text(email, style: style.copyWith(color: AppColors.onSurfaceVariant))),
          Expanded(flex: 1, child: Text(goals, style: style, textAlign: TextAlign.center)),
          Expanded(
            flex: 2,
            child: Text(
              checkins,
              style: style.copyWith(
                color: isHeader ? null : (isDone ? Colors.green : Colors.orange),
